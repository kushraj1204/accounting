...
class MY_Controller extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->library('neema_events');
    }

    protected function _ajaxCheck()
    {
        if (!$this->input->is_ajax_request()) {
            die("only ajax request allowed");
        }
    }
...
class Account_Controller extends MY_Controller
{
    ...
    function __construct()
    {
        ...
        $this->financial_year = (int)$this->session->userdata('account')['financial_year'];
        $skipArray = array('saveGeneralSettings', 'general', 'saveDateSettings');
        if (!in_array($this->router->method, $skipArray) && $this->financial_year == 0) {
            $this->session->set_flashdata('account_info', $this->lang->line('set_financial_year_first'));
