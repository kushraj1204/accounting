    ...
    public function mailsms($send_for, $sender_details, $date = NULL, $exam_schedule_array = NULL)
    {
        ...
        if (!empty($chk_mail_sms)) {
            if ($send_for == "student_admission") {
                ...
                $student = $this->CI->student_model->get($sender_details['student_id']);
                $subject = 'Payment received';
                $msg = $currency_symbol . ' ' . $sender_details['paid_amount'] . ' received as fee payment of ' . $student['firstname'] . ' ' . $student['lastname'] . ' for ' . $sender_details['bs_months'][$sender_details['fee_month'] - 1] . ' ' . $sender_details['fee_year'];
                if ($chk_mail_sms['mail']) {
                    $this->CI->mailgateway->sendMail($subject, $msg, $student['guardian_email']);
                    //$this->CI->mailgateway->sentAddFeeMail($invoice->invoice_id, $invoice->sub_invoice_id, $sender_details['email']);
                }
                if ($chk_mail_sms['sms']) {
                    $this->CI->smsgateway->sendSMS($student['guardian_phone'], $msg);
                }
            } elseif ($send_for == "absent_attendence") {

                $this->sendAbsentAttendance($chk_mail_sms, $sender_details, $date);
            } elseif ($send_for == "send_account_email") {
                if ($chk_mail_sms['mail']) {
                    $this->CI->mailgateway->sendAccountMail($sender_details['subject'], $sender_details['msg'], $sender_details['email']);
                }
                if ($chk_mail_sms['sms']) {
                    $this->CI->smsgateway->sendSMS($sender_details['contact_no'], $sender_details['msg']);
                }
            } else {
            }
        }
