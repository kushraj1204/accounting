...
class Customlib
{
    ...
    function __construct()
    {
        ...
        $this->CI->load->library('bikram_sambat');
        ...
        // $this->CI->load->model('Weekdays_model', '', true);
    ...
    {
        $status = array();
        $status['App_name'] = 'Eshikshya';
        $status['App_ver'] = 'Ver. 1.0';
        return $status;
    }

    function getDaysname($skip_holidays = false)
    {
        $settings = $this->CI->Setting_model->getSetting();
        $first_day = $settings->first_day;
        $p_holidays = $settings->public_holiday;
        $holidays = array_map(function ($d) {
            return strtolower($d);
        }, explode(', ', $p_holidays));
        $days = $this->CI->Weekdays_model->getDaysOfWeek($first_day);
        $status = array();
        foreach ($days as $day) {
            if ($skip_holidays && in_array($day['daykey'], $holidays)) {
                continue;
            }
            $status[ucfirst($day['daykey'])] = $day['dayname'];
        }
    ...
    {
        $months = array(
            4 => 'Baishakh (April/May)',
            5 => 'Jestha (May/June)',
            6 => 'Asar (June/July)',
            7 => 'Shrawan (July/August)',
            8 => 'Bhadra (August/September)',
            9 => 'Ashwin (September/October)',
            10 => 'Kartik (October/November)',
            11 => 'Mangshir (November/December)',
            12 => 'Poush (December/January)',
            1 => 'Magh (January/February)',
            2 => 'Falgun (February/March)',
            3 => 'Chaitra (March/April)',
        );
    ...
    function getNotificationModes()
    {
        ...
        $notification['send_account_email'] = $this->CI->lang->line('send_account_email');
        return $notification;
    }
    ...
    {
        return 'uploads/school_content/logo/';
    }

    public function getHeaderLogoPath()
    {
        return 'uploads/school_content/logo/header/';
    }

    public function getCurrentTime($format = 'Y-m-d H:i:s', $dateObj = false)
    {
    ...
    {
        return date($this->getSchoolDateFormat(), $this->dateyyyymmddTodateformat($date));
    }

    function addDaysReturnFormated($date, $day)
    {
        return date($this->getSchoolDateFormat(), strtotime($date . ' + ' . $day . 'days'));
    }

    function addDaysReturnStrtotime($date, $day)
    {
        $formatDate = $this->addDaysReturnFormated($date, $day);
        return strtotime($formatDate);
    }

    public function getBSDate($ad_date)
    {
        if (empty($ad_date)) {
            return '';
        }
        $val = DateTime::createFromFormat('Y-m-d', $ad_date);
        if ($val) {
            try {
                $this->CI->bikram_sambat->setEnglishDate($val->format('Y'), $val->format('n'), $val->format('j'));
                $bs_date = $this->CI->bikram_sambat->toNepaliString();
                return $this->getFormattedBSDate($bs_date);
            } catch (Exception $e) {
                return $e->getMessage() . ' ' . $ad_date;
            }
        }
        return 'invalid date:' . $ad_date;
    }

    public function getFormattedBSDate($value, $format = "y-m-d")
    {
        return $value;
    }

    public function getBSMonths()
    {
        return array(
            'Baisakh',
            'Jestha',
    ...
    public function getFeeDueDay()
    {
        $settings = $this->CI->Setting_model->getSetting();
        return $settings->fee_due_days;
    }

    public function getFeeGenerationDay()
    {
        $system_test = $this->CI->config->item('system_test');
        if ($system_test === '1') {
            //returns current day as generation day for test purpose
            $today = $this->getCurrentTime('Y-n-j');
            list($ad_year, $ad_month, $ad_day) = explode('-', $today);
            $system_calendar = $this->CI->config->item('system_calendar');
            if ($system_calendar == 'bs') {
                $this->CI->bikram_sambat->setEnglishDate($ad_year, $ad_month, $ad_day);
                $bs_date = $this->CI->bikram_sambat->toNepaliString();
                return explode('-', $bs_date)[2];
            } else {
                return $ad_day;
            }
        } else {
            $settings = $this->CI->Setting_model->getSetting();
            return $settings->fee_gen_day;
        }
    }
    //========================
}
