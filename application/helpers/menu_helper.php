...
function update_calendar_system($calendar_system) {
    $CI = &get_instance();
    $cs_path = APPPATH . 'config/system.php';
    $CI->load->helper('file');
    $CI->load->config('system');
    $cs_value = $CI->config->item('system_calendar');
    @chmod($cs_path, FILE_WRITE_MODE);
    $cs_file = file_get_contents($cs_path);
    $cs_file = trim($cs_file);
    $cs_file = str_replace("\$config['system_calendar'] = '" . $cs_value . "';", "\$config['system_calendar'] = '" . $calendar_system . "';", $cs_file);
    if (!$fp = fopen($cs_path, FOPEN_WRITE_CREATE_DESTRUCTIVE)) {
        return FALSE;
    }
    flock($fp, LOCK_EX);
    fwrite($fp, $cs_file, strlen($cs_file));
    flock($fp, LOCK_UN);
    fclose($fp);
    @chmod($cs_path, FILE_READ_MODE);
    return TRUE;
}

function update_jwt_installed() {
    $CI = &get_instance();
    $jwt_path = APPPATH . 'config/jwt.php';
    $CI->load->helper('file');
    $CI->load->config('jwt');
    $jwt_value = $CI->config->item('jwt_key');
    @chmod($jwt_path, FILE_WRITE_MODE);
    $jwt_file = file_get_contents($jwt_path);
    $jwt_file = trim($jwt_file);
    $jwt_key = bin2hex(random_bytes(16));
    $jwt_file = str_replace("\$config['jwt_key'] = '" . $jwt_value . "';", "\$config['jwt_key'] = '" . $jwt_key . "';", $jwt_file);
    if (!$fp = fopen($jwt_path, FOPEN_WRITE_CREATE_DESTRUCTIVE)) {
        return FALSE;
    }
    flock($fp, LOCK_EX);
    fwrite($fp, $jwt_file, strlen($jwt_file));
    flock($fp, LOCK_UN);
    fclose($fp);
    @chmod($jwt_path, FILE_READ_MODE);
    return TRUE;
}
