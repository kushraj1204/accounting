...
class User_model extends CI_Model {
    ...
    public function add($data) {
        if (isset($data['id'])) {
            ...
            if(isset($data['username'])) {
                $this->db->like('username', $data['username'], 'after');
                $this->db->from('users');
                $c = $this->db->count_all_results();
                if($c > 0) {
                    $data['username'] = $data['username'] . $c;
                }
            }
    ...
    public function addNewParent($data_parent_login, $student_data) {
        ...
        $this->db->like('username', $data_parent_login['username'], 'after');
        $this->db->from('users');
        $c = $this->db->count_all_results();
        if($c > 0) {
            $data_parent_login['username'] = $data_parent_login['username'] . $c;
        }
    ...
    public function read_user_information($users_id) {
        ...
        $this->db->select('ss.class_id,ss.section_id');
        $this->db->join('student_session ss','ss.student_id=users.user_id AND ss.session_id='.$this->db->escape($current_session),'inner');
