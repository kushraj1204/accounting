...
class Staff_model extends CI_Model
{
    ...
    public function valid_employee_id($str)
    {
        ...
        if ($this->check_data_exists($name, $id, $staff_id)) {
            $this->form_validation->set_message('check_exists', $this->lang->line('Record already exists'));
            return FALSE;
        } else {
            return TRUE;
        }
    ...
    public function searchFullTextInstructor($searchterm, $active)
    {

        $query = $this->db->query("SELECT `staff`.*, `staff_designation`.`designation` as `designation`, `department`.`department_name` as `department`,`roles`.`name` as user_type,`tbl_neema_user`.registration_code,tbl_neema_user.institute_code,tbl_neema_user.institute_userid,tbl_neema_user.institute_roleid  FROM `staff` LEFT JOIN `staff_designation` ON `staff_designation`.`id` = `staff`.`designation` LEFT JOIN `staff_roles` ON `staff_roles`.`staff_id` = `staff`.`id` LEFT JOIN `roles` ON `staff_roles`.`role_id` = `roles`.`id` LEFT JOIN `tbl_neema_user` ON `staff`.`id` = `tbl_neema_user`.`institute_userid` AND `tbl_neema_user`.institute_roleid = 6 LEFT JOIN `department` ON `department`.`id` = `staff`.`department` WHERE  `staff`.`is_active` = '$active' and `roles`.`id` = '2' and `staff`.`id` in (select staff_id from class_teacher) and (`staff`.`name` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`surname` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`employee_id` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`local_address` LIKE '%$searchterm%' ESCAPE '!'  OR `staff`.`contact_no` LIKE '%$searchterm%' ESCAPE '!' OR `staff`.`email` LIKE '%$searchterm%' ESCAPE '!')");
    ...
    function getStaffbyrole($id)
    {
        ...
        $this->db->join("roles", "staff_roles.role_id = roles.id", "inner");
        $this->db->where("staff_roles.role_id", $id);
        $this->db->where("staff.is_active", "1");
        $this->db->from('staff')->order_by('staff.name', 'asc');
        $query = $this->db->get();

        return $query->result_array();
    }

    public function getClassSectionAssociatedTeacher()
    {
        $sql = "select distinct ct.staff_id as staff_id,s.id as id,s.stn_title,s.name,s.surname,s.contact_no,s.employee_id from class_teacher ct 
    join staff s on ct.staff_id = s.id where s.is_active = ?";
    ...
    public function getByIdArray($ids = array(), $role)
    {
        $this->db->select("staff.*,staff_designation.designation,department.department_name as department, roles.id as role_id, roles.name as role");
        $this->db->from('staff');
        $this->db->join('staff_designation', "staff_designation.id = staff.designation", "left");
    ...
    public function checkDuplicate($idArray, $emailArray)
    {
        $emailquery = ' OR email IN  ( ' . implode(',', $emailArray) . ') ';
        $this->db->select('email,employee_id,id')->from('staff')
            ->where('employee_id IN  ( ' . implode(',', $idArray) . ')' . $emailquery);
        $query = $this->db->get();
        $data = $query->result();
        $dataArray = array();
        foreach ($data as $datum) {
            $dataArray['"' . $datum->email . '"'] = $datum->id;
            $dataArray['"' . $datum->employee_id . '"'] = $datum->id;
        }
        return array('count' => $query->num_rows(), 'data' => $dataArray);
    }

    public function batch_insert($data, $role, $leave_array = array())
    {
        ...
        unset($data['account_balance']);unset($data['account_balance_type']);
        ...
        $this->db->insert_batch('staff_roles', $insertData);
        ...
        $data['account_balance_type']=$account_balance_type;
        if($staff_id) {
            Neema_events::trigger('on_staff_create', array_merge($data, array('id' => $staff_id)));
        }
