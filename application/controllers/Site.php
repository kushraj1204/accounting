...
class Site extends Public_Controller {
    ...
    function login() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($result) {
                ...
                $session_data = array(
                    'id' => $result->id,
                    'username' => $result->stn_title . ' ' . $result->name . ' ' . $result->surname,
                    'email' => $result->email,
                    'roles' => $result->roles,
                ...
                if (isset($_SESSION['redirect_to']))
                 ...
                 $data['error_message'] = $this->lang->line('account_disabled');
                    
                  $this->load->view('admin/login', $data);
            }
               
            } else {
                $data['error_message'] = $this->lang->line('invalid_credential');
                $this->load->view('admin/login', $data);
            }
        }
    }
    ...
    function forgotpassword() {
        ...
        $this->form_validation->set_rules('email', 'lang:email', 'trim|valid_email|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($result && $result->email != "") {
                ...
                $this->session->set_flashdata('message', $this->lang->line("Please check your email to recover your password"));

                redirect('site/login', 'refresh');
            } else {
                $data = array(
                    'error_message' => $this->lang->line('Invalid Email')
                );
            }
