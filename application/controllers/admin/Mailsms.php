...
class Mailsms extends Admin_Controller {
    ...
    public function send_individual() {
        ...
        $this->form_validation->set_rules('individual_title', 'lang:title', 'required');
        $this->form_validation->set_rules('individual_message', 'lang:message', 'required');
        $this->form_validation->set_rules('user_list', 'lang:recipient', 'required');
        $this->form_validation->set_rules('individual_send_by[]', 'lang:send_through', 'required');
...
//                        }
                        if ($user_mail_value['mobileno'] != "") {
                            $mobile_no[] = $user_mail_value['mobileno'];
                        }
                    }
                    $mbno = implode(',',$mobile_no);
                    $response = $this->smsgateway->sendSMS($mbno, strip_tags($message));
                    if(isset($response['error']) && $response['error'] !== false) {
                        $escaped_data = json_encode(array('status' => 1, 'msg' => array($response['response'])));
//                        $escaped_data = json_encode(array('status' => 1, 'msg' => array($response['response'])), JSON_HEX_APOS|JSON_HEX_QUOT);
...
//                        $escaped_data = str_replace("\u0027", "\\'",  $escaped_data );
            ...
            echo json_encode(array('status' => 0, 'msg' => $this->lang->line('message_sent')));
        } else {
    ...
    public function send_group() {
        ...
        $this->form_validation->set_rules('group_title', 'lang:title', 'required');
        $this->form_validation->set_rules('group_message', 'lang:message', 'required');
        $this->form_validation->set_rules('user[]', 'lang:recipient', 'required');
        $this->form_validation->set_rules('group_send_by[]', 'lang:send_through', 'required');
        if ($this->form_validation->run()) {
            $user_array = array();
...
//                        }
    ...
    public function send_class() {
        ...
        $this->form_validation->set_rules('class_title', 'lang:title', 'required');
        $this->form_validation->set_rules('class_message', 'lang:message', 'required');
        $this->form_validation->set_rules('class_id', 'lang:class', 'required');
        $this->form_validation->set_rules('user[]', 'lang:recipient', 'required');
        $this->form_validation->set_rules('class_send_by[]', 'lang:send_through', 'required');
        if ($this->form_validation->run()) {
...
//                        foreach ($user_array as $user_mail_key => $user_mail_value) {
//                            if ($user_mail_value['email'] != "") {
//                                $email[] = $user_mail_value['email'];
//                            }
//                        }
//                        $ema = implode(',',$email);
//                        $this->mailer->send_mail($ema, $message_title, $message);
                    }
                }
                if ($send_sms) {
                    foreach ($user_array as $user_mail_key => $user_mail_value) {
