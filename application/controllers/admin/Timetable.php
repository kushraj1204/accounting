...
class Timetable extends Admin_Controller {
    ...
    function index() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data['class_teacher'] = $this->classteacher_model->getMainClassTeacher($class_id, $section_id);
                ...
                foreach ($result_subjects as $subject_k => $subject_v) {
                    ...
                    foreach ($getDaysnameList as $day_key => $day_value) {
                        ...
                        if (!empty($result)) {
                            ...
                            $obj->teacher_name = $result[0]['teacher_name'];
    ...
    function create() {
        ...
        $feecategory = $this->feecategory_model->get();
        $data['feecategorylist'] = $feecategory;
        $this->form_validation->set_rules('subject_id', 'lang:subject', 'trim|required|xss_clean');
        $this->form_validation->set_rules('class_id', 'lang:class', 'trim|required|xss_clean');
        $this->form_validation->set_rules('section_id', 'lang:section', 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            $this->load->view('layout/header', $data);
