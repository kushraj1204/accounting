...
class Payroll extends Admin_Controller {
    ...
    function index() {
        ...
        $data["name"] = "";
        $data['role']='';
        $data['monthlist_bs'] = $this->customlib->getBSMonths();
        $data['yearlist_bs'] = array(
            date("Y", strtotime("-1 year")) + 57,
            date('Y') + 57,
        );
        $now = $this->customlib->getCurrentTime('Y-m-d');
        $now_bs = $this->customlib->getBSDate($now);
        list($data['year'], $data['month'], $data['day']) = explode('-', $now_bs);
        $data['month'] = $data['monthlist_bs'][$data['month'] - 1];
        ...
        $submit = $this->input->get("search");
        if (isset($submit) && $submit == "search") {

            $month = $this->input->get("month");
            $year = $this->input->get("year");
            $emp_name = $this->input->get("name");
        ...
        $this->load->view("admin/payroll/stafflist_bs", $data);
...
//         }
    ...
    function paymentSuccess() {
        ...
        $date_bs = $this->input->post("payment_date_bs");
    ...
    function payrollreport() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->load->view("layout/header", $data);
            $this->load->view("admin/payroll/payrollreport_bs", $data);
            $this->load->view("layout/footer", $data);
        } else {
