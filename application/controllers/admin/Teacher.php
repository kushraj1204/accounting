...
class Teacher extends Admin_Controller {
    ...
    function assignteacher() {
        ...
        $teacher = $this->staff_model->getClassSectionAssociatedTeacher();
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            foreach ($loop as $key => $value) {
                ...
                $s['credit_hour'] = $this->input->post('credit_hour_id_' . $value);
    ...
    public function getSubjectTeachers() {
        ...
        if ($this->form_validation->run()) {
            ...
            $teachers = $this->classteacher_model->teacherByClassSection($class_id, $section_id);;

            echo json_encode(array('st' => 0, 'msg' => $data, 'teachers' => $teachers));
        } else {
            $data = array(
    ...
    function create() {
        ...
        $this->form_validation->set_rules('file', 'lang:photo', 'callback_handle_upload');
    ...
    function edit($id) {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->session->set_flashdata('msg', '<div class="alert alert-success text-center">'.$this->lang->line('saved_successfully').'</div>');
            redirect('admin/teacher/index');
        }
    }
...
//                )
//        );
        $this->form_validation->set_rules('class', 'lang:class', 'trim|required|xss_clean');
        $this->form_validation->set_rules('section', 'lang:section', 'trim|required|xss_clean');
        $this->form_validation->set_rules('teachers[]', 'lang:teacher', 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $teachers = $this->input->post("teachers");
            $class_teacher_id = $this->input->post('is_class_teacher');
            if(!empty($class_teacher_id)) {
                if(!in_array($class_teacher_id, $teachers)) {
                    array_push($teachers, $class_teacher_id);
                }
            }
            ...
            if(!empty($class_teacher_id)) {
                ...
                $this->db->where(array(
                    'class_id' => $class,
                    'section_id' => $section,
                ))->update('class_teacher', array('is_class_teacher' => 0));
                $this->db->where(array(
                    'class_id' => $class,
                    'section_id' => $section,
                    'staff_id' => $class_teacher_id,
                ))->update('class_teacher', array('is_class_teacher' => 1));
            }
    ...
    function classteacheredit($class_id, $section_id) {
        ...
        $assignteacherlist = $this->class_model->getClassTeacher();
        $is_class_teacher = array_filter($result, function ($i) {
            return $i['is_class_teacher']==1;
        });
        $data['class_teacher'] = current($is_class_teacher);
        $data['assignteacherlist'] = $assignteacherlist;
        foreach ($assignteacherlist as $key => $value) {
    ...
    function update_class_teacher() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $class_teacher_id = $this->input->post('is_class_teacher');
            if(!empty($class_teacher_id)) {
                if(!in_array($class_teacher_id, $staff_id)) {
                    array_push($staff_id, $class_teacher_id);
                }
            }
    ...
    function getTeacherByClassAndSection(){
        $class_id = $this->input->get('class_id');
        $section_id = $this->input->get('section_id');
        $data = $this->classteacher_model->distinctTeacherByClassSection($class_id, $section_id);
        echo json_encode($data);
    }
