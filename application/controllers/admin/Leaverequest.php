...
class Leaverequest extends Admin_Controller {
    ...
    function leaverequest() {
        ...
        if($this->datechooser == 'bs') {
            $this->load->view("admin/staff/staffleaverequest_bs", $data);
        } else {
...
//         }
        ...
        $leave_from = date($this->customlib->getSchoolDateFormat(), strtotime($result->leave_from));
        $result->leavefrom = $leave_from;
        $leave_to = date($this->customlib->getSchoolDateFormat(), strtotime($result->leave_to));
...
//         }
        ...
        $this->form_validation->set_rules('leavedates', 'Leave Date', 'trim|required|xss_clean|callback_leavedates_check');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $b = array_map('trim', explode('-', $a));

            $leavefrom = date("Y-m-d", strtotime($b[0]));
            $leaveto = date("Y-m-d", strtotime($b[1]));
            $applied_date_bs = $this->input->post('applied_date_bs');
            $leave_from_bs = $this->input->post('leave_from_bs');
            $leave_to_bs = $this->input->post('leave_to_bs');
            ...
            if (!empty($request_id)) {
                $data = array(
                    ...
                    'leave_to' => $leaveto,
                    'date_bs' => $applied_date_bs,
                    'leave_from_bs' => $leave_from_bs,
                    'leave_to_bs' => $leave_to_bs,
    ...
    public function leavedates_check($val)
    {
        $dates = array_map('trim', explode('-', $val));
        if(count($dates) != 2) {
            $this->form_validation->set_message('leavedates_check', 'Invalid leave start/end dates');
            return false;
        }
        $start = DateTime::createFromFormat($this->customlib->getSchoolDateFormat(), $dates[0]);
        $end = DateTime::createFromFormat($this->customlib->getSchoolDateFormat(), $dates[1]);
