...
class Feediscount extends Admin_Controller
{
    ...
    {
        parent::__construct();
        $this->load->event('discount_events');
    }

    function delete($id)
    {
        $data['title'] = 'feecategory List';
        $discount = $this->feediscount_model->get($id);
        if($discount) {
            $this->feediscount_model->remove($id);
            Neema_events::trigger('on_discount_delete', $discount);
        }
    ...
    function index()
    {
        ...
        $this->form_validation->set_rules('code', 'lang:discount_code', 'trim|required|xss_clean');
        $this->form_validation->set_rules('name', 'lang:name', 'trim|required|xss_clean');
        $this->form_validation->set_rules('type', $this->lang->line('fee_discount_type'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('value', $this->lang->line('fee_discount_value'), 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            ...
            $inserted_id = $this->feediscount_model->add($data);
            Neema_events::trigger('on_discount_add', array_merge($data, array('id' => $inserted_id)));
    ...
    function edit($id)
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            Neema_events::trigger('on_discount_edit', $data);
            $this->session->set_flashdata('msg', '<div class="alert alert-success">'.$this->lang->line('saved_successfully').'</div>');
            redirect('admin/feediscount/index');
        }
    }
