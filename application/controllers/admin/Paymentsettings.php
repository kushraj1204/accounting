...
class Paymentsettings extends Admin_Controller {
    ...
    function index() {
        ...
        $this->load->view('admin/payment_setting/prabhupay', $data);
        $this->load->view('layout/footer', $data);
    }

    public function prabhupay() {
        ...
        if ($this->form_validation->run()) {
            $data = array(
                'payment_type' => 'Prabhupay',
                'api_username' => $this->input->post('username'),
                'api_password' => $this->input->post('password'),
                'api_secret_key' => $this->input->post('secret'),
            );
            $this->paymentsetting_model->add($data);
            echo json_encode(array('st' => 0, 'msg' => $this->lang->line("Record Updated Successfully")));
        } else {
