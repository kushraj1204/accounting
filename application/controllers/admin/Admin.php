...
class Admin extends Admin_Controller
{
    ...
    function updateSession()
    {
        ...
        $session_array = array('session_id' => $session['id'], 'session' => $session['session'], 'session_bs' => $session['session_bs']);
    ...
    function backup()
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            if ($this->input->post('backup') == "upload") {
                $this->form_validation->set_rules('file', 'lang:file', 'callback_handle_upload');
    ...
    function changepass()
    {
        ...
        $this->form_validation->set_rules('current_pass', 'lang:current_password', 'trim|required|xss_clean');
        $this->form_validation->set_rules('new_pass', 'lang:new_password', 'trim|required|xss_clean|matches[confirm_pass]');
        $this->form_validation->set_rules('confirm_pass', 'lang:confirm_password', 'trim|required|xss_clean');
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($query1) {
                ...
                if ($check) {
                    ...
                    if ($query2) {
                        $data['error_message'] = "<div class='alert alert-success'>" . $this->lang->line("Password Changed SuccessFully") . "</div>";
                        $this->load->view('layout/header', $data);
                        $this->load->view('admin/change_password', $data);
                        $this->load->view('layout/footer', $data);
                    }
                } else {
                    $data['error_message'] = "<div class='alert alert-danger'>" . $this->lang->line("Invalid Current Password") . "</div>";
                    $this->load->view('layout/header', $data);
                    $this->load->view('admin/change_password', $data);
                    $this->load->view('layout/footer', $data);
                }
    ...
    function search()
    {
        ...
        $data['admin_search_text'] = trim($this->input->post('search_text'));
        $userdata = $this->customlib->getUserData();
        $carray = array();
