...
class ReceiptAndPayment extends Account_Controller
{
    ...
    public function index($financialYear = 0)
    {
        ...
        $this->session->set_userdata('sub_menu', 'account/receiptAndPayment');

        $type=$this->setting_model->getSchoolType();
        if($type->system_type==1)
        redirect("account/income_statement");
        ...
        $this->load->view('account/receiptAndPayment/index', $this->data);
